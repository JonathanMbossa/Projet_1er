...
<html lang="fr" dir="ltr">
    ...
    <div class="container">
      <div class="row">
        <?php
          ...
          $max = $db->query('SELECT max(id) AS _max FROM equipement'); //requête SQL pour connaitre la taille du tableau
          $req_max = $max->fetch(PDO::FETCH_ASSOC); //prend la première ligne renvoyer
          $min = $db->query('SELECT min(id) AS _min FROM equipement'); //requête SQL pour connaitre le debut du tableau
  ...
  <? include("includes/footer_catalogue.php"); ?>
  </body>
</html>
